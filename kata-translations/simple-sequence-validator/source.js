...
describe "Base Validate Sequence tests" do
  it "Static tests" do
...
Test.assert_equals(validate_sequence([100,200,300,400,500,600]), true)
  ...
  it "Auto tests" do
    truly = ->{
      ...
      Test.assert_equals(validate_sequence(arr.clone), s(arr.clone))
    }
    falsy = ->{
      arr = (0..rand(3..100)).to_a.shuffle
      Test.assert_equals(validate_sequence(arr.clone), s(arr.clone)) if arr != arr.sort && arr != arr.sort.reverse
    }
    450.times do
      1 == rand(1..2) ? truly[] : falsy[]
    end
  end
end
*/